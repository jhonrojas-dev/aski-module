/** @odoo-module **/
// ---------------------------------------------------------------------------
// VARIANTE ODOO 15 (rama 15.0) — OWL 1.4, no OWL 2.
// ---------------------------------------------------------------------------
// La 15 trae el web client nuevo (wowl) con sus registries, pero con OWL **1.4.11**:
//   * NO existe el modulo "@odoo/owl" (llego con OWL 2 en la 16). Si se importa,
//     Odoo descarta el modulo EN SILENCIO (lo loguea como `info`, no como error)
//     -> el widget simplemente no aparece y uno se vuelve loco buscando el bug.
//     OWL vive en el global `owl`, y los hooks bajo `owl.hooks`.
//   * NO existe `markup()`: el HTML crudo se pinta con `t-raw` (en OWL 2 es `t-out`).
//   * En las plantillas, `t-on-click` NO acepta arrow functions: hay que llamar al
//     metodo (`t-on-click="send()"`), si no el handler es un no-op.
